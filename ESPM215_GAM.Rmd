...
```{r}
lambda <- seq(0.0001, 0.003, by = 0.0001)
cv_score <- 0

for (i in 1:length(lambda)) {
  gam_init_fit <- gam(Sources ~ fStation + fMonth + s(Depth1000, sp = lambda[i]), data = ISIT2) 
  #cvFit_1 <- cvFit(gam_init_fit, data = d, y = d$Sources16, K = length(Sources16), foldType = "consecutive")
  cv_score[i] <- gam_init_fit$gcv.ubre
}

table <- data.frame(lambda, cv_score)
table

ggplot(data = table) +
  aes(x = lambda, y = cv_score) +
  geom_line()
```
